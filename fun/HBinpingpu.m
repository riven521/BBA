...
% %             end % END ISpingpuAll
            ...
            if ISpingpuShuaiWei==1 && ISshuaiwei==1
                ...
                %                步骤2:从车尾逐步找出对应Strip,降低其托盘的最大堆垛层数,进行reLoading,直到空间不够
...
fi = find(do.Strip.Strip_Bin(2,:) >= istrip );              % 同bin的strip序号 and 顺序>=istrip
...
% 计算LuIdx对应Lu的层数
function   curLuLayer = getLuLayer(LuIdx,do)
    curLuLayer = zeros(size(LuIdx));    
