...
if isSameCol(sItem)
    Item = getReorderStruct(Item.itemorder, sItem);
else
    error('不能使用structfun');
end
...
%% 函数1: getITEMorder
%% getITEMorder V2 % 给定ITEM的顺序,按NEXT FIT的方式插入STRIP
...
% V1 目前顺序 : 1: SID ; 2: isNonMixed;(相同LID下) 一般正真开始:    3: Longth/Height; 4:Width; 5: LID; (3,4,5,多数一样) 6: Height
...
% tmpItem = [ItemSIDord; Item.MixOrder; Item.MixOrder; ...                % MILKRUN VERSION 似乎也可用在 single version
%     Item.LWH(2,:); Item.LWH(1,:); ItemLID; Item.LWH(3,:); ItemEIDord; ];
...
% 不同whichStripH下,获得共同的thisLevel 默认3 nextfit
...
elseif p.whichStripH == 3 % nextfit
    % 增对Rotation增加变量  sItem.itemorder;    
...
%% 嵌套函数 insertItemToStrip v1
%     function insertItemToStrip(thisLevel,iItem)       
%    
% % %         % V2 按照Item的高度，高的优先进入Strip （此版本存在bug，4个间隙调整就有问题：有待调整）
