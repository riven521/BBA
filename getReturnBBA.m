...
% 参数2 - LU的长宽高(旋转后) % 以下是V3 - LU margin方式 LWH已经为减小长宽对应margin后的实际数据变量
output_LU_LWH = d.LU.LWH;  % DOUBLE LU的长宽高（旋转后：实际值）E LU的长宽高（旋转后）

%% 参数3 - GET sortedDaMax: 最小粒度单元LU展示的聚合
% 3.1 参数三的列排序 (暂未考虑LU_VehType)
    LU_Item=d.LU.LU_Item;
    LID=d.LU.LID;                   %LU堆垛用LUID, 但返回顺序用LID % LID=daMax.LU.ID;
    PID=d.LU.PID;
    SID=d.LU.SID;
    hLU=d.LU.LWH(3,:);
    LU_Bin = d.LU.LU_Bin;   %唯一两行的
    LU_VehType=d.LU.LU_VehType;

    if ~isfield(d.LU, 'isShuaiWei')
        LShuaiWei = zeros(length(LU_VehType),1)';
    else
        LShuaiWei = d.LU.isShuaiWei;
    end
...
%% 0 转变为Table: TLU
if ~isfield(d.LU, 'isShuaiWei'),    d.LU.isShuaiWei = zeros(length(d.LU.ID),1)'; end
