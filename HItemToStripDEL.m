...
clear tmpUniqueBin;
%% Item排序并初始化
    % getITEMorder - 获取ITEM的顺序(重点是高度/Rotation的最短边递减排序)
...
% printstruct(sortedITEMArray)
% printstruct(da)
LWHItemSort = sortedITEMArray.LWH;

    function order = getItemorder()
        if ParaArray.whichSortItemOrder == 1 %Descend of 长(高)
            [~,order] = sort(LWHItem(nDim,:),'descend');   % 按Item的（长度) 递减排序
...
%             [~,order] = sortrows(tmpLWH',[3 2 1],{'descend','descend','descend'}); %way1
            [~,order] = sort(tmpLWH(nDim+1,:),'descend'); %获取临时变量排序后的顺序 way2
        else
            error('设置参数错误');
        end
    end
    function item = getSortedITEM(itemorder)
        item = structfun(@(x) x(:,itemorder),da.ItemArray,'UniformOutput',false);
    end
...
% 获取itemorder: item的排序
    ...
    itemBeStripMatrix(:,da.ItemArray.order) = itemBeStripMatrixSort;
    CoordItemStrip(:,da.ItemArray.order) = CoordItemStripSort;
